...
import {CreditLoopController} from "src/CreditLoopController.sol";

contract CreditLoopControllerTest is Test {
    CreditLoopController private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_bondcredit");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new CreditLoopController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        CreditLoopController freshController = new CreditLoopController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
